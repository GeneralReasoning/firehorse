"""Static Claude Code tool descriptions for MCP-overridden tools.

When an environment provides tools that replace Claude Code built-ins (bash,
read, write, edit, grep, glob), we substitute the rich Claude Code descriptions
by default so the model retains behavioral guidance.

These descriptions are extracted from Claude Code's src/tools/*/prompt.ts and
cleaned of dynamic TypeScript (sandbox config, feature flags). They should be
periodically updated when Claude Code's prompts change significantly.

Cross-references use MCP tool names (mcp__openreward__*) with "if available"
